...
import {
	...
	ArtistSearch,
	PodcastSearch,
} from '../types/internal/search.types';
import { albumPayload } from './album.mapper';
import { artistMapPayload, artistMiniPayload } from './artist.mapper';
import { miniPayload } from './misc.mapper';
import { playlistPayload } from './playlist.mapper';
import { songPayload } from './song.mapper';

export function topSearchesPayload(s: SourceTopSearch): TopSearch {
	const {
		id,
		title,
		subtitle,
...
export function allSearchPayload(s: SourceAllSearch): AllSearch {
	const {
		albums: al,
		artists: ar,
		episodes: ep,
	...
	return {
		...
		songs: {
			...
			data: so.data.map(s => {
				const { id, title, type, image, url, description, album, ctr, position, more_info } = s;

				const { language, primary_artists, singers, triller_available } = more_info!;
		...
		playlists: {
			...
			data: pl.data.map(p => {
				const { id, title, type, image, description, url, language, extra, position, more_info } =
					p;
		...
		artists: {
			...
			data: ar.data.map(a => {
				const { id, title, url, type, image, description, ctr, entity, extra, position } = a;
		...
		top_query: {
			...
			data: tq.data.map(t => {
				const { id, title, type, image, description, url, album, ctr, position, more_info } = t;
				...
				const { primary_artists, singers, triller_available, language } = more_info;
		...
		shows: {
			...
			data: sh.data.map(s => {
				const { id, title, type, image, description, url, position, season_number } = s;
...
export function songSearchPayload(s: SourceSongSearch, mini: boolean = false): SongSearch {
	const { total, start, results } = s;

	return { total, start, results: results.map(s => songPayload(s, mini)) };
}

export function albumSearchPayload(a: SourceAlbumSearch, mini: boolean = false): AlbumSearch {
...
export function artistSearchPayload(a: SourceArtistSearch): ArtistSearch {
	const { total, start, results } = a;
...
export function podcastsSearchPayload(p: SourcePodcastSearch): PodcastSearch {
