...
import {
	...
	SourceArtistDetails,
	SourceSimilarArtist,
} from '../types/external/artist.types';
import { Artist, ArtistSong, ArtistDetails, SimilarArtist } from '../types/internal/artist.types';
import { ArtistMap, ArtistMini } from '../types/common.types';
import { albumPayload } from './album.mapper';
import { miniPayload } from './misc.mapper';
import { playlistPayload } from './playlist.mapper';
import { songPayload } from './song.mapper';

export function artistPayload(a: SourceArtist, mini: boolean = false): Artist {
	const {
		artistId: id,
	...
	return {
		...
		top_albums: topAlbums?.map(a => (mini ? miniPayload(a) : albumPayload(a))) ?? [],
		dedicated_artist_playlist:
			dedicated_artist_playlist?.map(p => (mini ? miniPayload(p) : playlistPayload(p))) ?? [],
		featured_artist_playlist:
			featured_artist_playlist?.map(p => (mini ? miniPayload(p) : playlistPayload(p))) ?? [],
		...
		is_radio_present,
		bio: JSON.parse(bio || '[]') as Artist['bio'],
		dob,
		fb,
		twitter,
		wiki,
...
export function similarArtistPayload(a: SourceSimilarArtist): SimilarArtist {
	const {
		id,
		name,
		roles,
	...
	return {
		id,
		name: decode(name),
		roles: JSON.parse(roles || '{}') as SimilarArtist['roles'],
		aka,
		fb,
		twitter,
		wiki,
		similar: JSON.parse(similar || '[]') as SimilarArtist['similar'],
		dob,
		image: createImageLinks(image_url),
		search_keywords,
		primary_artist_id,
		languages: JSON.parse(languages || '{}') as SimilarArtist['languages'],
		url,
		type,
		is_radio_present,
		dominant_type,
...
export function artistSongPayload(a: SourceArtistSong): ArtistSong {
	const {
		explicit_content,
		id,
		image,
...
export function artistDetailsPayload(a: SourceArtistDetails, mini: boolean = false): ArtistDetails {
	const {
		artistId: id,
		dominantLanguage: dominant_language,
		dominantType: dominant_type,
	...
	return {
		...
		top_albums: topAlbums
			? {
					...
					albums: topAlbums.albums.map(a => (mini ? miniPayload(a) : albumPayload(a))),
				}
			: undefined,
	};
}

export function artistMapPayload(a: string | SourceArtistMap): ArtistMap {
	return typeof a === 'string'
		...
		: {
				...
				featured_artists: dedupArtists(a.featured_artists?.map(artistMiniPayload) ?? []),
				primary_artists: dedupArtists(a.primary_artists.map(artistMiniPayload)),
			};
}

export function artistMiniPayload(a: SourceArtistMini): ArtistMini {
