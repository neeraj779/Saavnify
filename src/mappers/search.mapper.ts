...
export const mapSearchResponse = (search: SearchAPIResponse): Search => ({
	topQuery: {
...
export const mapSearchPlaylistResponse = (playlist: SearchPlaylistAPIResponse): SearchPlaylist => ({
...
export const mapSearchAlbumResponse = (album: SearchAlbumAPIResponse): SearchAlbum => ({
	...
	results: album.results.map(item => ({
		...
		explicitContent: item.explicit_content === '1',
		artists: {
			primary: item.more_info?.artistMap?.primary_artists?.map(mapArtistMapResponse),
			featured: item.more_info?.artistMap?.featured_artists?.map(mapArtistMapResponse),
			all: item.more_info?.artistMap?.artists?.map(mapArtistMapResponse),
		},
		image: createImageLinks(item.image),
	})),
});
