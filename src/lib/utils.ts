...
import { MediaQuality } from '../types/common.types';
import { ArtistMini } from '../types/common.types';
import { SourceArtistMini } from '@/types/external/artist.types';
 ...
 */
export function createImageLinks(link: string): MediaQuality {
 ...
 */
export function createDownloadLinks(encryptedMediaUrl: string): MediaQuality {
	const qualities = [
...
type Artist = (ArtistMini | SourceArtistMini)[];
...
export function dedupArtists(artists: Artist) {
	const uniqueArtists: { [id: string]: ArtistMini } = {};
